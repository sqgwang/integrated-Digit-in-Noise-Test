...
async function loadList() {
  ...
  const el = $("list");
  if (!data.items.length) { el.innerHTML = '<p class="note">No records.</p>'; return; }

  el.innerHTML = `
    <table class="table">
      <thead>
        <tr>
          <th>Time</th><th>PID</th><th>StimLang</th>
          <th>Conditions</th><th>SRTs</th><th>Actions</th>
        </tr>
      </thead>
      <tbody>
        ${data.items.map(r => {
  ...
  el.querySelectorAll("button.view").forEach(btn => {
    btn.addEventListener("click", async () => {
      try { showDetail(await api("/api/results/" + btn.dataset.id)); }
  ...
  if (isAdmin) {
    el.querySelectorAll("button.del").forEach(btn => {
      btn.addEventListener("click", async () => {
        ...
        try { await api("/api/results/" + btn.dataset.id, { method:"DELETE" }); loadList(); }
...
async function loadUsers() {
  try {
    ...
    $("userList").innerHTML = users.map(u => `${u.username} (${u.role})`).join(" · ");
...
window.addEventListener("DOMContentLoaded", async () => {
  $("btnLogin").addEventListener("click", async () => {
    ...
    try {
      const body = JSON.stringify({ username: $("loginUser").value.trim(), password: $("loginPass").value });
      await api("/api/auth/login", { method:"POST", body });
      boot();
    } catch { $("loginMsg").textContent = "Login failed."; }
  });
  ...
  $("btnAddUser").addEventListener("click", async () => {
    $("userMsg").textContent = "";
    try {
      const body = JSON.stringify({ username: $("newUser").value.trim(), password: $("newPass").value, role: $("newRole").value });
      await api("/api/users", { method:"POST", body });
      $("newUser").value = ""; $("newPass").value = "";
      await loadUsers();
      $("userMsg").textContent = "User created.";
    } catch (e) { $("userMsg").textContent = "Create failed: " + e.message; }
  });
...
function showDetail(rec) {
  ...
  meta.className = "note";
  meta.textContent = `PID: ${rec.participantId || rec.userInfo?.pid || "—"} · Lang: ${rec.meta?.stimLang || rec.userInfo?.stimLang || "—"} · Trials: ${(rec.trials || []).length}`;
  ...
  btnCSV.onclick = () => {
    ...
    const headers = ["participantId","condition","nDigits","digitsPresented","presentedSNR","effectiveSNR","response","correct","rt_ms","timestamp"];
    const rows = [headers.join(",")].concat(trials.map(t => headers.map(h => JSON.stringify(t[h] ?? "")).join(",")));
